...
const Post = ({ post }) => {
  console.log(post);
  let { handlelikepost, handleunlikepost,handleFollow } = usePost()
  return (
    <div className='post'>
      <div className='user'>
        <div className='left'>
  ...
  ) : post.isFollowed?.status === "pending" ? (
    <button onClick={() => handleFollow(post)}>
      Requested
    </button>
  ) : (
    <button onClick={() => handleFollow(post)}>
      ...
      <div className='options'>
        <div className='left'>
          {post.isLiked
            ? (
              <button onClick={() => { post.isLiked ? handleunlikepost(post._id, post.userid.username) : handlelikepost(post._id, post.userid.username) }} className='red'>
            ...
            : (
              <button onClick={() => { post.isLiked ? handleunlikepost(post._id, post.userid.username) : handlelikepost(post._id, post.userid.username) }}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12.001 4.52853C14.35 2.42 17.98 2.49 20.2426 4.75736C22.5053 7.02472 22.583 10.637 20.4786 12.993L11.9999 21.485L3.52138 12.993C1.41705 10.637 1.49571 7.01901 3.75736 4.75736C6.02157 2.49315 9.64519 2.41687 12.001 4.52853Z" />
                </svg>
              </button>
